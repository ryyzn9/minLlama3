...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.wg = nn.Linear(args.dim, args.n_heads * self.head_dim, bias=False)
        self.group_norm = nn.GroupNorm(args.n_heads, self.head_dim)

        self.swish = lambda x: x * torch.sigmoid(x)

        self.dropout = nn.Dropout(0.05)
        self.gammas = (1 - torch.exp(torch.linspace(math.log(1/32), math.log(1/512), args.n_heads))).detach().cpu().tolist()
    ...
    def forward(
        ...
        D = [self._get_D(self.new_method(seqlen),gamma=gamma).to(device='cuda') for gamma in self.gammas]
       ...
       # Convert the list of PyTorch tensors to a single PyTorch tensor
        ...
        scores = torch.matmul(xq, keys.transpose(2, 3)) * D.unsqueeze(0)
        # if mask is not None:
